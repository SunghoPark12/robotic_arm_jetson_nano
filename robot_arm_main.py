...
def main():
    #go_grip(249,-41,3,-40,0)
    
    go_pose(*HOME_POSE)
    #detector class
    detector = Detector(engine_path="yolo1280.engine", device="cuda:0")
    ...
    #print("실측 좌표(mm) 입력")
    #camera position fixed. Need not to click pixel point!
    #pixel_points = [(41, 104),(229, 105),(412, 106),(585, 106),(762, 108),(933, 104),(1108, 101),
    #(42, 284),(231, 284),(409, 283),(588, 283),(762, 281),(933,278),(1110,276),
    #(40,466),(232,463),(413,460),(589,458),(767,456),(936,453),(1114,451),
    #(45,644),(229,640),(410,637),(590,634),(768,631),(941,630),(1117,629)]
    pixel_points = [(34,123), (216,120), (396, 119), (570, 117), (744,113), (918,109),(1094,102),
    (39,299), (221,295), (398,294), (576, 286), (749, 283), (921,278),(1099, 273),
    (43,477), (226, 471), (405, 465), (579, 460), (755, 457), (927, 452), (1105, 448),
    (45,655), (225,648), (405,642), (582,636), (759, 633), (934, 628), (1112, 624)]
    
    #camera angle change -> 8th column delete: (339,-41), (339,-11), (339,19), (339,49)
    real_points = [(129,-41),(159,-41),(189,-41),(219,-41),(249,-41),(279,-41),(309,-41),
    (129,-11),(159,-11),(189,-11),(219,-11),(249,-11),(279,-11),(309,-11),
    (129,19),(159,19),(189,19),(219,19),(249,19),(279,19),(309,19),
    (129,49),(159,49),(189,49),(219,49),(249,49),(279,49),(309,49)]
    
    #no more need(real points directly typed)
    ...
    while True:
        ...
        cv2.putText(frame, "ESC to quit calibration", (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)
    ...
    #main loop
    while (True):
        cls, pixel_x, pixel_y = detector.obj_pixel(cap, window_name, 5)
        z_chosen = z_cls(cls) #proper z value for the class
    
        #if (grip_rot): # checking object's HVR(horizontal, vertical ratio)
        #    rot_angle = 0
        #else:
        #    rot_angle = -90
        ...
        time.sleep(1)
        go_grip(real_x,real_y,z_chosen,-60,0)
        go_lay_down(cls)
